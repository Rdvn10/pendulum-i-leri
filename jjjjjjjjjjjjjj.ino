...
case SWING_UP:
      ...
      // --- SWING-UP (ENERJİ POMPALAMA) MODU ---
       double kinetik = eylemsizlik_moment * sarkac_hiz * sarkac_hiz / 2.0;
       double potansiyel = m * l * g * (1 - cos(sarkac_konum));
       mevcut_enerji = potansiyel + kinetik;

       double enerji_hata = mevcut_enerji - hedef_enerji;
       V_uygulanacak = enerji_hata * Ke * sign(sarkac_hiz * cos(sarkac_konum));

       // Swing-up limitleri (Eskiden 15 pals = 54 Derece. 4X'te -> 60 Pals)
       if (motor_hamkonum >= 100 && V_uygulanacak > 0) V_uygulanacak = 0; 
       else if (motor_hamkonum <= -100 && V_uygulanacak < 0) V_uygulanacak = 0; 

       pwm_uygulanacak = abs(V_uygulanacak) * 255.0 / 12.0; 
       pwm_uygulanacak = constrain(pwm_uygulanacak, 20, 120); 

        // GÖREV: Sarkaç yatayın altındaysa enerji pompala
         {
           //Titremeyi kesen ölü bölge mantığı
           double itki_yonu;
          if (fabs(sarkac_hiz) < 0.05) itki_yonu = 0.0;
           else itki_yonu = (sarkac_hiz > 0) ? 1.0 : -1.0;
          
           double aci_carpani = cos(sarkac_konumrad); // DÜZELTME: pals değil radyan
           double merkezleme_torku = -0.8 * motor_konumrad - 0.15 * motor_hiz;
           V_uygulanacak = (1.1 * itki_yonu * aci_carpani) + merkezleme_torku;
           ...
           pwm_uygulanacak = (pwm_ham < 8) ? 0 : constrain(pwm_ham, 30, 80);
           pwm_uygulanacak = 0; // test durdur 
         }
         break;
...
case TAMPON_BOLGE:
        ...
        // --- GÜVENLİK VE ÇIKIŞ ---
        // Donanımsal sınır koruması (Zaten yazmışsınız)
        ...
        // Eğer hesaplanan PWM 0'dan büyükse ama motoru döndüremeyecek kadar küçükse, destek ver.
        if (pwm_ham > 0) { 
...
motorgucver(V_uygulanacak, pwm_uygulanacak);
  ...
  Serial.print("\t S_Hiz:"); 
  Serial.print(sarkac_hiz * rad2deg, 2); // println'i print yaptık
  Serial.print("\t PWM:"); 
  Serial.println(pwm_uygulanacak); // Yeni eklenen PWM ve satır atlama (println)
  }
}